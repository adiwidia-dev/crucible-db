...
The generated password is shown exactly once. Crucible stores only an Argon2id-compatible Laravel password hash. If the user loses it, **Rotate credentials** revokes the old lease credential, disconnects every active socket, creates a new password, and displays it once.
...
8. Device codes, tunnel tokens, and synthetic passwords are random, independently scoped, separately hashed, short-lived, and never interchangeable.
