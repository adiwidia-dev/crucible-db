...
- Query Access grants a time-bounded browser session over one or more connections. The requester chooses read-only or read + write access, and the backend enforces it for every session query. Each execution accepts exactly one statement; users can select one statement in the SQL editor to run it with the **Run selected** action or `Cmd/Ctrl + Enter`.
