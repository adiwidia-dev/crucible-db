...
Use this documentation to request and review SQL work, grant time-bounded browser access, configure workspace policy, and operate the application safely.
...
Crucible DB supports PostgreSQL and MySQL targets through its own browser workflows. It is not a network proxy for tools such as pgAdmin, DBeaver, or the `psql` command line client. It does not turn a deployment batch into a user-controlled transaction: transaction-control SQL remains blocked.
