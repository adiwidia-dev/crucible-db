...
Navigation is grouped by Work, Data, and Manage, with Account anchored in the footer. Manage contains an independently collapsible Administration tree, which further groups Access & identity, Security & policy, Application, and Governance. Each subsection keeps its own manual disclosure state, and Administration and Account can remain open together. The top bar carries a compact notification control and account identity with direct sign-out. Default items remain quiet, hover gains a neutral surface, and the active item uses a pale neutral background with an Action Blue icon. On mobile, navigation becomes a sheet while preserving the same hierarchy.
...
- **Do** organize navigation around Work, Data, Manage, and the footer Account disclosure.
